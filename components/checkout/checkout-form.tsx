...
export function CheckoutForm() {
  ...
  const handlePlaceOrder = async () => {
    ...
    try {
      ...
      // Use API for reliable server-side order creation
      const response = await fetch("/api/orders", {
        ...
        body: JSON.stringify({
          ...
          items: validItems.map((item) => ({
            ...item,
            id: item.menuItemId,
            spiceLevel: items.find((i) => i.menuItem.id === item.menuItemId)?.spiceLevel || null,
          ...
          deliveryAddress: orderType === "delivery" ? { street, city, postalCode } : null,
          note: orderNote.trim() || notes || null,
          metadata: {
            couponCode: couponCode || null,
            discount: discount > 0 ? Number.parseFloat(discount.toFixed(2)) : 0,
            branchId: dineInParams?.branchId || "default-branch",
          },
        }),
      })
  ...
  const handlePaymentComplete = async () => {
    ...
    try {
      // Use API to update payment status
      const response = await fetch(`/api/orders/${pendingOrderId}/status`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
