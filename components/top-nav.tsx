...
export function TopNav({ isWrapped = false }: { isWrapped?: boolean }) {
  ...
  return (
    <header className="fixed top-0 left-0 right-0 z-50 w-full bg-white border-b border-gray-200">
      {/* Safety Block - Guaranteed 44px min height for iPhone status bar */}
      <div 
        className="w-full" 
        style={{ height: 'max(44px, var(--safe-top, 44px))' }} 
      />
      {content}
    </header>
  )
